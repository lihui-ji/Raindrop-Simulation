...
Foam::tmp<Foam::pointField> Foam::solidBodyMotionSolver::curPoints(vector displacement)
{
    if (moveAllCells_)
    {
        return transformPoints(SBMFPtr_().transformation(displacement), points0_);
    }
    else
    {
        tmp<pointField> ttransformedPts(new pointField(mesh().points()));
        ...
        UIndirectList<point>(transformedPts, pointIDs_) = transformPoints
        (
            SBMFPtr_().transformation(displacement),
            pointField(points0_, pointIDs_)
        );
